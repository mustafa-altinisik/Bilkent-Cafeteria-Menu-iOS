...
class ModifyNotificationsViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, NotificationsTableViewCellDelegate {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        guard let notifications = notifications, indexPath.row < notifications.count else {
            return
        }

        let selectedNotification = notifications[indexPath.row]
        let notificationDetailScreen = NotificationDetailsViewController(nibName: "NotificatonDetailsViewController", bundle: nil)
        notificationDetailScreen.notificationToBeModified = selectedNotification
        notificationDetailScreen.modalPresentationStyle = .fullScreen
        present(notificationDetailScreen, animated: true, completion: nil)    }
